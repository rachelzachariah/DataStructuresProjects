...
public class User {
	...
	public void addToPendingCheckIn(Document doc, Change.Type type, String repoName){
		...
		ChangeSet changeset = new ChangeSet(repoName);
		changeset.addChange(doc, type);
		pendingCheckIns.add(pendingCheckIns.size(), changeset);
	}
	...
	public ErrorType checkIn(String repoName) {
		...
		if (VersionControlDb.findRepo(repoName)!=null){
			for (int i=0; i<pendingCheckIns.size(); i++){
				if (pendingCheckIns.get(i).getReponame().equals(repoName)){
					VersionControlDb.findRepo(repoName).queueCheckIn(pendingCheckIns.get(i));
	...
	public ErrorType checkOut (String repoName) {
		...
		if(subRepos.contains(repoName)){
			...
			if (repo!=null){
				RepoCopy repocopy = new RepoCopy(repoName,repo.getVersion(),repo.getDocuments());
				for (int i=0; i<workingCopies.size(); i++){
					if (workingCopies.get(i).getReponame().equals(repoName)) workingCopies.remove(i);
				}		
				workingCopies.add(repocopy);
				return ErrorType.SUCCESS;	
			}
			else return ErrorType.REPO_NOT_FOUND;
		}
